...
- Maneuvers (paras 207-218) — Bite the Bullet (BA: Risk Die + Gunslinger level temp HP), Blindfire (BA: 30 ft Blindsight until end of turn), Dodge Roll (BA: 15 ft move + reload, ignores OAs and difficult terrain), Grazing Shot (no action: on miss, deal Risk Die + DEX mod weapon-type damage 1/turn), Maverick Spirit (no action: failed INT/WIS/CHA check or save adds Risk Die 1/turn), Skin of Your Teeth (Reaction: on hit, add Risk Die to AC). The existing `dodge_roll` and `blindfire` menu entries are stubs that consume Risk Dice but need accurate effects.
