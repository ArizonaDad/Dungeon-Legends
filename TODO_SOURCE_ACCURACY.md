...
- **Vestiges of Divergence** (Wildemount) — SHIPPED data + auto-advancement. 8 source-quoted legendary items with Dormant/Awakened (L9-15)/Exalted (L16+) states in `Server/combat/vestiges_data.nvgt` per source paras 9058-9219. State auto-advances by character level via `apply_equipment_bonuses()` (`character_data.nvgt`); L9/L16 crossings trigger an announcement in `send_level_up_choices()` (`battle_manager.nvgt`). **Loot catalog gap:** only Hide of the Feral Guardian is currently in `loot_data.nvgt` as a droppable item (id `wildemount_hide_feral_guardian`). The other 7 vestiges (spell_bottle, danoths_visor, grimoire_infinitus, infiltrators_key, stormgirdle, verminshroud, wreath_of_prism) need loot entries before they can drop. Mechanical effects only wired for hide_feral_guardian (AC bonus by state) and stormgirdle (lightning resistance) — the other 6 need their dormant/awakened/exalted ability text wired into combat once added to loot.
