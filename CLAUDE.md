...
- **Save Failure Reaction Chain**: At each condition-applying save failure site (22 total), the chain fires in order: `try_countercharm_reroll` → `try_fanatical_focus` → `try_disciplined_survivor_reroll` → `try_flash_of_genius` → `try_maverick_spirit` → `try_mage_slayer_guarded_mind` → `try_boon_of_fate_bonus` → `try_stroke_of_luck_save` → condition application. All smart-spend (only fire when they could actually help).
